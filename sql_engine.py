...
from util import read_meta, check_for, error_exit, format_string
...
def process_query(query, table_info):
    ...
    for table in tables:
        if table not in table_info.keys():
            error_exit('No Such Table Exists')
    ...
    if len(clauses) > 1 and len(tables) == 1:
        ...
        process_where(clauses[1], columns, tables, table_info)
    elif len(clauses) > 1 and len(tables) > 1:
        # Join Condition
        process_where_join(clauses[1], columns, tables, table_info)
    elif len(tables) > 1:
        process_join(columns, tables, table_info)
    elif len(function_process) != 0:
        for query in function_process:
            process_function(query[0], query[1], tables, table_info)
    else:
        process_project(columns, tables, table_info)
